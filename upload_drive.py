...
def upload_files(folder_id, local_path):
    ...
    for file_path in files_to_upload:
        ...
        try:
            # Buscar si el archivo ya existe en la carpeta (incluyendo Unidades Compartidas)
            ...
            if not items:
                # Subir nuevo archivo (especificando soporte para Unidades Compartidas)
                ...
                # Actualizar archivo existente (especificando soporte para Unidades Compartidas)
                file_id = items[0]['id']
            ...
            # Intentar verificar si la carpeta existe para dar mejor info
            try:
                folder = service.files().get(fileId=folder_id, supportsAllDrives=True).execute()
                logging.info(f"La carpeta destino existe y es accesible: {folder.get('name')} (ID: {folder.get('id')})")
            except Exception as folder_error:
                logging.error(f"CRÍTICO: No se puede acceder a la carpeta destino con ID: {folder_id}. Verifica los permisos de la Service Account y que el ID sea correcto. Error: {folder_error}")
            raise e
...
if __name__ == '__main__':
    ...
    if not FOLDER_ID:
        ...
        # Limpieza básica del ID por si se copió con espacios
